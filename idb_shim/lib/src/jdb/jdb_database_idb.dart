...
import 'package:idb_shim/src/jdb/web_defs.dart';
...
class JdbDatabaseIdb implements jdb.JdbDatabase {
  ...
  jdb.JdbReadEntryEncoded _encodedEntryFromCursor(CursorWithValue cwv) {
    var map = cwv.value as Map;

    // Deleted is an int in jdb
    var deleted = map[idbDeletedKey] == 1;

    var key = map[idbKeyKey] as jdb.Key;
    ...
    var entry = jdb.JdbReadEntryEncoded(
      id,
      storeName,
      key,
      deleted,
  ...
  Future<int?> _txnAddEntries(
    idb.Transaction txn,
    Iterable<jdb.JdbWriteEntryEncoded> entries,
  ) async {
  ...
  @override
  Future<jdb.StorageJdbWriteResult> writeIfRevision(
    jdb.StorageJdbWriteQuery query,
  ) async {
    ...
    return jdb.StorageJdbWriteResult(
