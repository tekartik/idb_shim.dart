...
void sdbTypeTest(SdbTestContext ctx) {
  ...
  group('type codec none', () {
    var dbName = 'type_test_codec_none.db';
    Future<void> open() async {
      db = await factory.openDatabase(
        dbName,
        options: SdbOpenDatabaseOptions(
    ...
    test('timestamp', () async {
      var ref = anyTypeStore.record('timestamp');
      await expectLater(
        () async => await ref.put(db, SdbTimestamp.now()),
        throwsA(isA<Object>()),
      );
    });
    test('blob', () async {
      var ref = anyTypeStore.record('bloc');
      await expectLater(
        () async => await ref.put(db, SdbBlob.fromList([1, 2, 3])),
        throwsA(isA<Object>()),
      );
    });
  });
