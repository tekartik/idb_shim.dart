...
void sdbIndexTests(TestContext ctx) {
  ...
  group('sdb_index', () {
    ...
    test('filter int db', () async {
      ...
      var records = await testIndex.findRecords(db, filter: filter);
  ...
  group('index2', () {
    test('index2', () async {
      ...
      await db.inStoreTransaction(itemStore, SdbTransactionMode.readWrite, (
        ...
        keyCatAlbert = await itemStore.add(txn, {
          'type': 'cat',
          'name': 'Albert',
        });
        keyDogBeethoven = await itemStore.add(txn, {
          'type': 'dog',
          'name': 'Beethoven',
        });
        keyCatHarriet = await itemStore.add(txn, {
          'type': 'cat',
          'name': 'Harriet',
        });
      });
      ...
      items = await itemTypeIdIndex.findRecords(
        db,
    ...
    test('index3', () async {
      ...
      items = await index.findRecords(
        db,
        options: SdbFindOptions(
          boundaries: SdbBoundaries.lower(
            SdbLowerBoundary(('a', 1, 'j'), include: false),
          ),
        ),
      );
    ...
    test('index4', () async {
      ...
      await db.inStoreTransaction(store, SdbTransactionMode.readWrite, (
        ...
        key2 = await store.add(txn, {
          'col1': 'b',
          'col2': 1,
          'col3': 'i',
          'col4': 3,
        });
        key3 = await store.add(txn, {
          'col1': 'a',
          'col2': 1,
          'col3': 'j',
          'col4': 4,
